...
Create an Azure resource group by using an Azure Resource Manager template.
Create an Azure VM by using an Azure Resource Manager template.
Task 1: Create an Azure resource group by using an Azure Resource Manager template
On SEA-ADM1, start Microsoft Edge, browse to the Azure portal, and authenticate with your Azure credentials.
In the Azure portal, open a PowerShell session in the Cloud Shell pane.
Upload the C:\Labfiles\Lab04\L04-sub_template.json file to the Cloud Shell home directory.
From the Cloud Shell pane, run the following commands to create a resource group that will contain resources you provision in this lab. (Replace the <Azure region> placeholder with the name of an Azure region into which you can deploy Azure virtual machines, such as eastus.)
...
From the Cloud Shell pane, upload an Azure Resource Manager template C:\Labfiles\Lab04\L04-rg_template.json and the corresponding Azure Resource Manager parameter file C:\Labfiles\Lab04\L04-rg_template.parameters.json.
From the Cloud Shell pane, run the following command to deploy an Azure VM running Windows Server, which you’ll be using in this lab:
...
In the Azure portal, add GatewaySubnet with the 10.4.3.224/27 IP address range to the az800l04-vnet virtual network.
...
Create an Azure Network Adapter.
...
If prompted, in the Windows Security dialog box, enter the credentials provided by the instructor, and then select OK.

From the Windows Admin Center page, attempt to add an Azure Network Adapter.
When prompted, register Windows Admin Center to the Azure subscription you used in the previous exercise.
Task 2: Create an Azure Network Adapter
On SEA-ADM1, in the Microsoft Edge window displaying Windows Admin Center, attempt to create an Azure Network Adapter again.
Create an Azure Network Adapter with the following settings:
...
On SEA-ADM1, switch to the Microsoft Edge window displaying the Azure portal and verify that a new virtual network gateway with the name starting with WAC-Created-vpngw- is being provisioned.
...
Install Windows Admin Center gateway in Azure.
Review results of the script provisioning.
Task 1: Install Windows Admin Center gateway in Azure
On SEA-ADM1, switch to the browser window displaying the the Azure portal.
In the Azure portal, start a PowerShell session in the Cloud Shell pane.
From the Cloud Shell pane, upload the file C:\Labfiles\Lab04\Deploy-WACAzVM.ps1 into the Cloud Shell home directory.
From the Cloud Shell pane, run the following command to enable the compatibility for the AzureRm PowerShell cmdlets that are used by the Windows Admin Center provisioning script:

code
Enable-AzureRmAlias -Scope Process
Run the following commands to set values of variables necessary to run the the Windows Admin Center provisioning script (Replace the <Azure region> placeholder with the name of an Azure region into which you deploy resources earlier in this lab, such as eastus.):
...
Run the following command to set the script parameters variable:
...
Run the following commands to disable certificate verification for PowerShell remoting (when prompted following the first command, enter A and press the Enter key):

code
install-module pswsman
Disable-WSManCertVerification -All
Run the following command to launch the provisioning script:

code
./Deploy-WACAzVM.ps1 @scriptParams
When prompted to provide the name for the local Administrator account, enter Student.
When prompted to provide the password for the local Administrator account, enter Pa55w.rd1234.

Note: Wait for the provisioning script to complete. This might take about 5 minutes.

Verify that the script completed successfully and note the final message providing the URL containing the fully qualified name of the Azure VM that hosts the Windows Admin Center installation.
...
In the Azure portal, browse to the page of the AZ800-L0401-RG resource group.
On the AZ800-L0401-RG page, on the Overview page, review the list of resources, including the Azure VM az800l04-vmwac.
On the **az800l04-vmwac	Networking** page, on the Inbound port rules tab, note entries representing the inbound port rule allowing connectivity on TCP port 5986 and the inbound rule allowing connectivity on TCP port 443.
...
Connect to the Windows Admin Center gateway running in Azure VM.
Enable PowerShell Remoting on an Azure VM.
Connect to an Azure VM by using the Windows Admin Center gateway running in Azure VM.
Task 1: Connect to the Windows Admin Center gateway running in Azure VM
On SEA-ADM1, start Microsoft Edge and connect the Windows Admin Center gateway running in the Azure VM you identified in the previous exercise.
When prompted, sign in with the Student username and Pa55w.rd1234 password.
On the All connections pane of Windows Admin Center, select az800l04-vmwac [Gateway].
Examine the Overview pane of Windows Admin Center.
Task 2: Enable PowerShell Remoting on an Azure VM
On SEA-ADM1, in the Microsoft Edge window displaying the Azure portal, browse to the page of the az800l04-vm0 Azure VM.
From the az800l04-vm0 page, use the Run Command feature to run the following command to enable Windows Remote Management in case it is disabled:

code
winrm quickconfig -quiet
Use the Run Command feature to run the following command to open Windows Remote Management inbound port:

code
Set-NetFirewallRule -Name WINRM-HTTP-In-TCP-PUBLIC -RemoteAddress Any
Use the Run Command feature to run the following command to enable PowerShell Remoting:

code
Enable-PSRemoting -Force -SkipNetworkProfileCheck
Task 3: Connect to an Azure VM by using the Windows Admin Center gateway running in Azure VM
On SEA-ADM1, in the Microsoft Edge window displaying the interface of the Windows Admin Center gateway running on the az800l04-vmwac Azure VM, add a connection to the Azure VM az800l04-vm0 by using its name.
When prompted, authenticate by using the credentials provided by the instructor.
After successfully connecting to the VM, examine the Overview pane of the az800l04-vmwac Azure VM in Windows Admin Center.
...
From the Azure portal, open a PowerShell session in the Cloud Shell pane.
Task 2: Identify all Azure resources provisioned in the lab
From the Cloud Shell pane, run the following command to list all resource groups created throughout this lab:

code
Get-AzResourceGroup -Name 'az800l04*'
Run the following command to delete all resource groups you created throughout this lab:
